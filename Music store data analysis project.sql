...
select cte.customer_id,cust.first_name,cust.last_name,round(cte.totals,2) total_spending from cte join customer cust
on cte.customer_id=cust.customer_id;
 ...
 select cte1.artist_id,art.name,cte1.total_track_count from cte1
...
# 9.Find how much amount spent by each customer on artists?Write a query to return customer_name,artist_name and total_spent
 ...
 select c.customer_id,c.first_name,c.last_name,bsa.name,round(sum(l.quantity*l.unit_price),2) as totalspent
 from customer c
...
select c.country as country,count(il.quantity) as purchases,g.genre_id as genre_id,g.name as genre_name,
...
group by country,genre_id,genre_name
order by country,purchases desc
)
select country,purchases,genre_id,genre_name from most_popular_genre
where rn<=1;
